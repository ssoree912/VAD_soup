...
def _collect_split(
    ...
    video_sequence: Optional[Sequence[str]] = None,
    ...
    if video_sequence:
        videos = []
        for name in video_sequence:
            path = frames_root / name
            if not path.is_dir():
                print(f"[warn] Frames for video {name} missing under {frames_root}; skipping.")
                continue
            videos.append(path)
    else:
...
def main():
    ...
    ap.add_argument("--split_file", type=str, default=None,
                    help="Optional CSV (video,label,frames) to lock processing order.")
    args = ap.parse_args()
    ...
    if args.split_file:
        ...
        with open(split_path, "r") as fh:
            split_lines = [ln.strip() for ln in fh.readlines() if ln.strip()]
        video_order = []
        for line in split_lines:
            parts = [p.strip() for p in line.split(",") if p.strip()]
            if not parts:
                continue
            video_order.append(parts[0])
    boxes_arr, classes_arr = _collect_split(frames_root, detections_root, video_order)
